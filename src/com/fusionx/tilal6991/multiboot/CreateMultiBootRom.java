...
public class CreateMultiBootRom extends Activity {
	...
	private class CreateMultibootRomAsync extends
			...
			try {
				...
				while (scanner.hasNextLine()) {
					...
					if (findString.contains("format(")
						...
						CommonFunctions.findAndReplaceInFile(updaterScript,
								findString, "");
		...
		private void packUpAndFinish() {
			...
			CommonFunctions.writeToFile(finalOutdir + "boot" + romName + ".sh",
					shFile);
			...
			if (!findTextInFile(tempSdCardDir + "currentRom.init.rc",
				...
				CommonFunctions.writeToFile(finalOutdir + "boot.sh", shFile);
			}
		}
		...
		private void remakeBootImage() {
			...
			CommonFunctions.findAndReplaceInFile(tempSdCardDir + "init.rc",
					"on fs", "on fs\n" + "    mkdir -p " + externalDir + "\n"
							+ "    mount vfat /dev/block/mmcblk0p1 "
							+ externalDir, true);
